...
# 超大代码块降级阈值（防止一次性高亮导致 DOM/主线程压力过大）
LARGE_CODE_BLOCK_CHAR_THRESHOLD = 120 * 1024
LARGE_CODE_BLOCK_LINE_THRESHOLD = 1500
LARGE_CODE_BLOCK_PREVIEW_LINES = 120
...
def _can_access_document_metadata(
    filename, metadata, user=None, allow_password_access=False
):
    ...
    password_rule_access = bool(
        allow_password_access and user is None and has_password_rule_access(filename)
    )
...
def _normalize_metadata(filename, raw_metadata, raw_content):
    ...
    title = str(
        ...
        or os.path.splitext(basename)[0].replace("-", " ").replace("_", " ").strip()
        or basename
    )
    ...
    metadata = {
        ...
        "cover": normalize_local_media_url(
            str(raw_metadata.get("cover") or "").strip()
        ),
        ...
        "category_name": str(
            raw_metadata.get("category_name") or _format_category_name(category)
        ).strip(),
        "filename": filename,
...
def _find_large_fenced_code_blocks(content):
    """检测 Markdown 中超大 fenced code block，并返回其区间与语言信息。"""
    text = str(content or "")
    ...
    for idx, line in enumerate(lines):
        ...
        if not in_block:
            ...
            fence_char = fence[0]
            fence_len = len(fence)
            lang = (open_match.group("lang") or "").strip().split(" ")[0].strip()
        ...
        if re.match(close_pattern, line):
            ...
            is_large = (
                char_count >= LARGE_CODE_BLOCK_CHAR_THRESHOLD
                or line_count >= LARGE_CODE_BLOCK_LINE_THRESHOLD
            )
            if is_large:
                blocks.append(
                    {
...
def _build_large_code_placeholder(block, source_filename="", block_index=0):
    """构建超大代码块占位 HTML：默认仅展示预览片段，避免一次性渲染全部代码。"""
    code_text = block.get("code_text") or ""
    all_lines = code_text.splitlines()
    preview_lines = all_lines[:LARGE_CODE_BLOCK_PREVIEW_LINES]
    preview_text = "\n".join(preview_lines)
    if code_text.endswith("\n") and preview_lines:
        preview_text += "\n"

    escaped_preview = html.escape(preview_text, quote=False)
    lang = str(block.get("lang") or "").strip()
    line_count = int(block.get("line_count") or 0)
    char_count = int(block.get("char_count") or 0)
    omitted_lines = max(0, line_count - len(preview_lines))

    language_attr = f' data-language="{html.escape(lang, quote=True)}"' if lang else ""
    language_label = (
        f'<span class="code-language-badge">{html.escape(lang)}</span>' if lang else ""
    )
    meta = f"{line_count} 行 · {char_count} 字符"
    if omitted_lines > 0:
        meta += f" · 已折叠 {omitted_lines} 行"

    safe_filename = html.escape(str(source_filename or ""), quote=True)
    safe_block_index = int(block_index or 0)

    return (
        '<div class="code-block-wrapper large-code-block is-truncated"'
        f' data-large-code="1"{language_attr} data-lines="{line_count}" data-chars="{char_count}"'
        f' data-source-file="{safe_filename}" data-code-block-index="{safe_block_index}">'
        '<div class="code-toolbar large-code-toolbar">'
        f"{language_label}"
        '<span class="large-code-meta">'
        f"{meta}"
        "</span>"
        "</div>"
        '<pre class="has-code-toolbar"><code>'
        f"{escaped_preview}"
        "</code></pre>"
        "</div>"
    )
...
def _prepare_markdown_for_render(content, source_filename=""):
    """
    预处理 Markdown：
    - 普通内容保持原样（继续使用 codehilite）。
    - 超大 fenced code block 替换为安全占位 HTML，避免高亮阶段生成巨量 DOM。
    """
    text = str(content or "")
    blocks = _find_large_fenced_code_blocks(text)
    if not blocks:
        return text
...
def _render_markdown(content, source_filename=""):
...
def _attach_view_counts(items):
    if not items:
        return items
    filename_map = {
        item.get("filename"): item for item in items if item.get("filename")
    }
    if not filename_map:
        return items
    stats = DocumentViewStat.query.filter(
        DocumentViewStat.filename.in_(list(filename_map.keys()))
    ).all()
    for stat in stats:
...
def _parse_markdown_file(filename):
    ...
    html, toc_html = _render_markdown(raw_content, source_filename=normalized_filename)
...
def _extract_first_image(content):
    ...
    html_match = re.search(
        r'<img[^>]+src=["\']([^"\']+)["\']', content or "", re.IGNORECASE
    )
    if html_match:
...
def _build_front_matter(raw_metadata, body_content, filename):
    ...
    if "cover" in raw_metadata:
        ...
        if cover_value in ("none", "false", "__none__", ""):
            ...
            # 用户设置了具体的封面 URL
            metadata["cover"] = normalize_local_media_url(
                str(raw_metadata.get("cover")).strip()
            )
    else:
        # cover 字段不存在，尝试自动提取
        metadata["cover"] = normalize_local_media_url(
            str(_extract_first_image(body_content) or "").strip()
        )
    ...
    if metadata.get("template") == "post":
        metadata["slug"] = _ensure_unique_post_slug(
            metadata.get("slug") or file_slug, filename
        )
    ...
    front_matter = yaml.safe_dump(
        {key: value for key, value in ordered if value not in (None, "", [])},
        allow_unicode=True,
        sort_keys=False,
    ).strip()
    return f"---\n{front_matter}\n---\n\n{body_content.lstrip()}"
...
def get_markdown_files():
    ...
    def build_tree(current_rel_path=""):
        ...
        current_dir = (
            ...
            else resolve_docs_path(current_rel_path, allow_directory=True)[2]
        )
        ...
        for item in items:
            ...
            item_rel_path = (
                os.path.join(current_rel_path, item).replace("\\", "/")
                if current_rel_path
                else item
            )
            ...
            if os.path.isdir(item_path):
                ...
                if not children and not check_permission(
                    current_user, item_rel_path, "upload"
                ):
                    continue
                tree.append(
                    {
...
def get_default_file_for_dir(docs_root, relative_dir_path=""):
    ...
    _, relative_dir_path, abs_dir_path = resolve_docs_path(
        relative_dir_path, allow_directory=True
    )
    ...
    dir_config = DirectoryConfig.query.filter_by(
        dir_path=relative_dir_path or "/"
    ).first()
    md_files = [
        filename
        for filename in os.listdir(abs_dir_path)
        if os.path.isfile(os.path.join(abs_dir_path, filename))
    ...
    md_files = [
        ...
        if _has_read_permission(
            os.path.join(relative_dir_path, filename).replace("\\", "/").lstrip("/")
        )
    ]
    visible_md_files = []
    for filename in md_files:
        ...
        if not payload:
            continue
        if not _can_access_document_metadata(
            relative_filename, payload.get("metadata") or {}, allow_password_access=True
        ):
            continue
    ...
    if dir_config:
        if dir_config.default_open_file and dir_config.default_open_file in md_files:
            return (
                os.path.join(relative_dir_path, dir_config.default_open_file)
                .replace("\\", "/")
                .lstrip("/")
            )
        ...
        return (
            os.path.join(relative_dir_path, md_files[0]).replace("\\", "/").lstrip("/")
        )

    lower_files = {filename.lower(): filename for filename in md_files}
    if "readme.md" in lower_files:
        return (
            os.path.join(relative_dir_path, lower_files["readme.md"])
            .replace("\\", "/")
            .lstrip("/")
        )
    if "index.md" in lower_files:
        return (
            os.path.join(relative_dir_path, lower_files["index.md"])
            .replace("\\", "/")
            .lstrip("/")
        )
...
def get_public_post_documents():
    ...
    for filename in _iter_markdown_filenames():
        ...
        item["timeline_display"] = (
            item.get("updated_display") or item.get("date_display") or "未设置日期"
        )
        items.append(item)
    ...
    if filenames:
        filename_map = {
            item.get("filename") or item.get("path"): item for item in items
        }
        stats = DocumentViewStat.query.filter(
            DocumentViewStat.filename.in_(filenames)
        ).all()
        for stat in stats:
            target = filename_map.get(stat.filename)
            if target is not None:
...
def get_all_tags(include_private=False):
    ...
    tags = sorted(
        tag_map.values(), key=lambda item: (-item["count"], item["name"].lower())
    )
    if not tags:
        return []
        ...
        archive_groups.append(
            {
                ...
                "range_label": f"{year} 年 {int(month)} 月"
                if year and month
                else label,
...
def get_public_post_tree(include_private=False):
    ...
    def ensure_dir(path_value):
        ...
        node = {
            ...
            "name": name,
            "path": normalized,
            "url": f"/category/{quote(normalized)}",
            "children": [],
        }
...
def search_posts(query, include_private=False):
    ...
    for filename in _iter_markdown_filenames():
        ...
        if keyword in haystack:
            item = dict(metadata)
            item["search_snippet"] = _build_search_snippet(
                payload.get("raw_content") or "", keyword
            )
            results.append(item)
