...
from flask import Blueprint, Response, current_app, jsonify, request, session, url_for
from flask_login import current_user, login_required

from models import Image, ShareLink, SystemSetting, User, db
...
def _get_app_timezone():
    timezone_name = (
        current_app.config.get("APP_TIMEZONE") or "Asia/Shanghai"
...
def _get_share_for_api(token, require_edit=False, require_access=False):
    ...
    if (
        require_access
        and share_link.is_password_protected
        and not session.get(build_share_session_key(share_link.token))
    ):
...
def _resolve_share_target_for_owner(target_type, target_path, allow_edit=False):
    if target_type == "file":
        ...
        if allow_edit and not check_permission(
            current_user, normalized_target_path, "edit"
        ):
            return (
                None,
                None,
                jsonify({"error": "没有该文档的编辑权限，无法创建可编辑分享"}),
                403,
            )
    else:
        _, normalized_target_path, absolute_target_path = resolve_docs_path(
            target_path, allow_directory=True
        )
        ...
        if allow_edit and not check_permission(
            ...
            return (
                None,
                None,
                jsonify({"error": "没有该目录的编辑权限，无法创建可编辑分享"}),
                403,
            )
...
def _get_owned_share_link(token):
    share_link = ShareLink.query.filter_by(
        token=(token or "").strip(), created_by_user_id=current_user.id
    ).first()
    if not share_link:
...
def suggest_users():
    ...
    like_value = f"{query}%"
    users = (
        User.query.filter(User.username.like(like_value))
    ...
    return jsonify(
        {
            "items": [
                {"username": user.username, "nickname": user.nickname or ""}
                for user in users
            ]
        }
    )
...
def get_raw_markdown():
    ...
    if not payload or not _can_access_document_metadata(
        filename, payload.get("metadata") or {}
    ):
        return jsonify({"error": "Permission denied"}), 403
...
@api_bp.route("/code_block_full")
def get_full_code_block():
    filename = (request.args.get("filename") or "").strip()
    block_index_raw = (request.args.get("block_index") or "").strip()
    ...
    try:
        ...
        return jsonify({"error": "Invalid block_index"}), 400
    ...
    lines = str(raw_content).splitlines(keepends=True)
    in_block = False
    fence_char = ""
    fence_len = 0
    lang = ""
    ...
    for line in lines:
        ...
        close_pattern = (
            r"^\s*" + re.escape(fence_char) + r"{" + str(fence_len) + r",}\s*$"
        )
        if re.match(close_pattern, line):
            ...
            is_large = char_count >= 120 * 1024 or line_count >= 1500

            if is_large:
                matched_count += 1
                if matched_count == block_index:
                    return jsonify(
                        {
                            "content": code_text,
                            "language": lang,
...
def save_markdown():
    ...
    if not check_permission(current_user, filename, "edit"):
        return jsonify(
            {"error": "Permission denied: no edit access for this directory"}
        ), 403
    ...
    try:
        ...
        if not check_permission(current_user, filename, "manage"):
            ...
            if existing_state != submitted_state:
                return jsonify(
                    {
                        "error": "Permission denied: no manage access for front matter changes"
                    }
                ), 403

        content = sync_front_matter(
            content, filename, ensure_front_matter=ensure_front_matter
        )
...
def check_front_matter_slug():
    ...
    conflicts = _find_post_slug_conflicts(
        normalized_slug, exclude_filename=normalized_filename
    )
    suggested_slug = _suggest_unique_post_slug(
        normalized_slug or "post", exclude_filename=normalized_filename
    )
    return jsonify(
        {
            "success": True,
            ...
            "items": [
                {
                    ...
                    "post_url": (
                        url_for("main.post_detail", slug=item.get("slug"))
                        if item.get("template") == "post" and item.get("slug")
                        else ""
                    ),
        ...
        return jsonify(
            {
                ...
                "post_url": (
                    ...
                    if (raw_metadata.get("template") or "doc") == "post"
                    and raw_metadata.get("slug")
                    else ""
                ),
            }
        )
...
def media_upload():
    ...
    if (
        not check_permission(current_user, "", "upload")
        and current_user.role != "admin"
    ):
        return jsonify(
            {"error": "Permission denied: you do not have upload rights."}
        ), 403
    ...
    if len(file.read()) > max_size_mb * 1024 * 1024:
        return jsonify(
            {"error": f"File exceeds the maximum size of {max_size_mb}MB"}
        ), 413
    file.seek(0)  # 重置文件指针
...
def upload_markdown():
    ...
    try:
        ...
        _, target_dir, abs_target_dir = resolve_docs_path(
            target_dir, allow_directory=True, create_directory=True
        )
    except InvalidPathError:
    ...
    if not check_permission(current_user, target_dir, "upload"):
        return jsonify(
            {"error": "Permission denied: no upload access for this directory"}
        ), 403
...
def rename_markdown_document():
    ...
    if not check_permission(current_user, source_path, "edit"):
        return jsonify(
            {"error": "Permission denied: no edit access for this file"}
        ), 403
...
def delete_markdown_document():
    ...
    if not check_permission(current_user, filename, "delete"):
        return jsonify(
            {"error": "Permission denied: no delete access for this file"}
        ), 403
...
def create_directory():
    ...
    try:
        ...
        _, parent_dir, abs_parent_dir = resolve_docs_path(
            parent_dir, allow_directory=True, create_directory=True
        )
    except InvalidPathError:
...
def rename_directory():
    ...
    try:
        _, source_path, abs_source_dir = resolve_docs_path(
            source_path, allow_directory=True
        )
    ...
    _, target_path, abs_target_dir = resolve_docs_path(
        target_path, allow_directory=True
    )
...
def delete_directory():
    ...
    if not check_permission(current_user, dirname, "delete"):
        return jsonify(
            {"error": "Permission denied: no delete access for this directory"}
        ), 403
...
def create_share():
    ...
    share_link.title = build_share_title(
        target_type, normalized_target_path, target_name
    )
...
def list_shares():
    ...
    if scope == "all":
        share_links = query.order_by(
            ShareLink.created_at.desc(), ShareLink.id.desc()
        ).all()
        return jsonify(
            {
                "success": True,
                "items": [
                    _build_share_response(share_link) for share_link in share_links
                ],
            }
        )

    try:
        normalized_target_path, _, error_response, status_code = (
            _resolve_share_target_for_owner(
                target_type,
                target_path,
                allow_edit=False,
            )
...
def list_my_shares():
    share_links = (
        ShareLink.query.filter_by(created_by_user_id=current_user.id)
        .order_by(ShareLink.created_at.desc(), ShareLink.id.desc())
        .all()
    )
    return jsonify(
...
def get_share_raw(token):
    share_link, error_response, status_code = _get_share_for_api(
        token, require_edit=True, require_access=True
    )
    if error_response is not None:
        return error_response, status_code
    ...
    try:
        normalized_target_path, absolute_target_path, _ = resolve_shared_path(
            share_link, target_relative_path
        )
    except InvalidPathError:
        return jsonify({"error": "分享路径无效"}), 400

    if not normalized_target_path.endswith(".md") or not os.path.isfile(
    ...
    with open(absolute_target_path, "r", encoding="utf-8") as file_obj:
        content = file_obj.read()
    return jsonify(
        {"success": True, "content": content, "path": normalized_target_path}
    )
...
def save_shared_document(token):
    ...
    try:
        final_content = sync_front_matter(
            content, normalized_target_path, ensure_front_matter=ensure_front_matter
        )
...
def delete_image_by_filename(unique_filename):
    ...
    if not success:
        status_code = (
            ...
            or error == "Orphan file not found in local storage."
            else 400
        )
    ...
    try:
        ...
        for filename in _iter_markdown_filenames():
            ...
            if not _can_access_document_metadata(
                filename, metadata, allow_password_access=True
            ):
                continue
            ...
            if query_lower in searchable_text:
                # 生成搜索摘要
                content = re.sub(
                    r"^---[\s\S]*?---\s*", "", raw_content or "", flags=re.MULTILINE
                )
